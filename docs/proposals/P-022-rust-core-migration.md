...
**Preferred queue:** #259 remaining (cp4 → cp5) → #260/#269. One Python-first
side change is queued ahead of nothing and blocks nothing: a documented
defensive limit on source-coordinate integers, which closes the last measured
cp1 divergence (Python accepts coordinates past 64 bits; Rust does not) without
pushing arbitrary-precision integers through the Rust core.
