...
| 7a | dual-engine shadow mode + zero-diff reproduction artifacts | #260 (supported by #269) | **infrastructure underway — checkpoints 1–2 landed**, final acceptance **blocked by #259**. cp1 (same-input `OwnIR` capture + canonical hash, and the reproduction-artifact format) and cp2 (the **engine protocol**: each engine reports its per-layer outputs in one format and declares what it could *produce*) are built and replayed with zero of the other engine's runtime: `ownlang/repro.py` is the reference emitter, `own-shadow` the port's half. **80 documents** across five corpora digest-pinned and re-hashed by the port **0/0/0**, **9** artifacts round-tripped byte-for-byte and self-verifying, **80** tamper controls, **6** executable domain refusals, **18** structural + **5** backstop negative controls per side, campaigns **30/30** and **11/11** caught ([cp1](../notes/p022-shadow-infra-checkpoint1.md), [cp2](../notes/p022-shadow-infra-checkpoint2.md), [generated census](../generated/p022-shadow-census.md)). The port's verdict layer declares the **cp4 projection** rather than emitting a short document a comparison would score as agreement; the four layer envelopes where the two engines' statuses differ are all **declared** boundaries (the OBL protocol analysis, and the typed door). One **finding**, recorded and resolved as a contract decision rather than by bending either engine: the literal `-0` reads as an integer in CPython's `json` and a float in `serde_json`, so it is refused by both, at the literal level, under an executable ledger. Remaining: the `AnalysisTrace` schema + stable-ID normalization (#269), and first-divergence reduction over the *lowered*/MOS layers. **Not declarable as shadow mode**, and not a parity claim — the artifacts pair the two engines' captures and **nothing compares them**; acceptance compares end diagnostics |
