...
**Preferred queue:** **#262's Stage 3 has landed — Rust is the public default on every launcher surface, Python is the tested rollback, and Stage 4 (Python distribution removal) has not started.** P-022 is therefore NOT complete: Stage 4 is open, #263 is open and still owes the performance baselines this decision explicitly deferred, and the compare gates are retained as development/CI evidence. With the cutover complete the P-022 feature freeze on verdict-changing inference lifts, which opens the **P-037 A1** production gate — not started here, and deliberately not started in the same change as a cutover.
