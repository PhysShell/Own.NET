...
| 7a | dual-engine shadow mode + zero-diff reproduction artifacts | #260 (supported by #269) | **infrastructure underway — checkpoints 1–3 landed**, final acceptance **blocked by #259**. cp1 (same-input `OwnIR` capture + canonical hash, and the reproduction-artifact format), cp2 (the **engine protocol**: each engine reports its per-layer outputs in one format and declares what it could *produce*) and cp3 (the **`AnalysisTrace`** #269: stable-ID normalization, with each layer's ordering semantics *declared* rather than normalized away) are built and replayed with zero of the other engine's runtime: `ownlang/repro.py` is the reference emitter, `own-shadow` the port's half. **80 documents** across five corpora digest-pinned and re-hashed by the port **0/0/0**, **9** artifacts round-tripped byte-for-byte carrying both engines' captures, **54** trace layers / **254** addressed steps reproduced byte-for-byte by an independent implementation, **80** tamper controls, **6** executable domain refusals, **18** structural + **5** backstop negative controls per side, campaigns **30/30**, **11/11** and **11/11** caught ([cp1](../notes/p022-shadow-infra-checkpoint1.md), [cp2](../notes/p022-shadow-infra-checkpoint2.md), [cp3](../notes/p022-shadow-infra-checkpoint3.md), [generated census](../generated/p022-shadow-census.md)). Two **findings**, each recorded and resolved as a contract decision rather than by bending either engine: the literal `-0` reads as an integer in CPython's `json` and a float in `serde_json` (refused by both, at the literal level, under an executable ledger); and the two independent readings of the trace schema disagreed on where a duplicate address takes its `~n` suffix, exposing an inconsistency in the schema itself. Remaining: first-divergence reduction over the *lowered*/MOS layers. **Not declarable as shadow mode**, and not a parity claim — the artifacts pair the two engines' captures and the trace makes them walkable, but **nothing compares them**; acceptance compares end diagnostics |
