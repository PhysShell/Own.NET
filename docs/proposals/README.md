...
| [P-022](P-022-rust-core-migration.md) | Rust core migration: crate DAG, patterns, prior art, differential oracle (Python = golden) | in execution — steps 0–4 built (`own-ir`/`own-syntax`/`own-cfg`/`own-diagnostics`/`own-analysis`, #214/#249); step 6a done (`spec/Bridge.md` + `spec/BridgeBehaviorMatrix.md`, #258); step 6b underway (`own-lowered`/`own-bridge`: lowering + MOS-summaries parity landed, analysis wiring open, #259); next ready step is 5a (#255); Python authoritative until cutover |
