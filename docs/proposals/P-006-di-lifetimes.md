...
1. Where to anchor the diagnostic — the registration line, the consuming
   ...
   "expected: Window — actual: App — path: …".)** For the captive family (DI001/DI002/DI003)
   the finding keeps its **primary** anchor at the registration site and names the
   **consuming constructor** — where the captive is injected — both in the message tail
   (`[consumed by the '<singleton>' constructor at <file>:<line>]`) and as a structured
   **SARIF `relatedLocation`** (clickable, cross-file). The extractor records each
   implementation's ctor location (the widest public ctor, or the class declaration for a
   primary/implicit ctor); the core appends it when known and degrades cleanly when not.
