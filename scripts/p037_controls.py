...
``--post-a1``: check ``post_a1`` (the acceptance A1 discharges; expected to
FAIL until A1 lands). Non-production: reads the controls, runs the launcher,
changes nothing. Exit 0 on all-match, 1 otherwise.

ENGINE (#262 Stage 3). The launcher is invoked with an EXPLICIT ``--engine``
and never bare. Two reasons, and neither is the census:

* since the cutover a bare invocation resolves the Rust candidate, so on a
  machine with no candidate it exits 2 before doing any work — a control that
  cannot reach an engine measures nothing;
* the ``current`` record above was taken at ``70189a3``, when a bare
  invocation still meant Python. Leaving the invocation bare would have
  SILENTLY changed which engine the record describes, which is exactly the
  rewriting-of-history the record exists to prevent.

The default is ``rust``: the public default since Stage 3 and the engine A1
implements guarded summaries in. ``--engine python`` measures the reference,
``--engine both`` measures each independently against the same record and
reports whether they agreed. ``both`` agreeing is an OBSERVATION, not a
contract — A1 changes the Rust engine and not Python, so a future divergence
here is a finding to classify, not a failure to paper over.
"""
...
SCHEMAS = ("p037-control-expectation/1", "p037-control-expectation/2")


def own_check(control: Path, facts: Path, engine: str) -> str:
...
def measure(control: Path, engine: str, call_site_line: int) -> tuple[list[str], bool | None]:
    """One control under one engine: (finding codes, fabricated release at the call site)."""
    with tempfile.TemporaryDirectory() as tmp:
        facts = Path(tmp) / "facts.json"
        text = own_check(control, facts, engine)
        codes = sorted({m.group(2) for m in FINDING.finditer(text)})
        fabricated = call_site_line in release_lines(facts) if facts.exists() else None
    return codes, fabricated


def check(control: Path, post_a1: bool, engine: str) -> tuple[bool, list[str]]:
    """Compare one control against its recorded expectation; (ok, report lines)."""
    spec: dict[str, Any] = json.loads((control / "expected.json").read_text(encoding="utf-8"))
    if spec.get("schema") not in SCHEMAS:
        return False, [f"{'BAD SCHEMA':8} {control.name:42} schema={spec.get('schema')!r}"]
    ...
    call_site_line = int(spec["call_site_line"])

    engines = ["python", "rust"] if engine == "both" else [engine]
    seen: dict[str, tuple[list[str], bool | None]] = {}
    lines: list[str] = []
    all_ok = True
    for eng in engines:
        codes, fabricated = measure(control, eng, call_site_line)
        seen[eng] = (codes, fabricated)
        ok = codes == want_codes and fabricated == want_fab
        all_ok &= ok
        lines.append(
            f"{'ok' if ok else 'MISMATCH':8} {control.name:42} [{spec['classification']}] "
            f"engine={eng:6} findings={codes} (want {want_codes})  "
            f"fabricated release at line {call_site_line}={fabricated} (want {want_fab})"
        )
    if len(engines) > 1:
        agreed = len(set(map(repr, seen.values()))) == 1
        lines.append(
            f"{'':8} {control.name:42} engines "
            + ("AGREE (the seam is decided in the extractor, before either engine runs)"
               if agreed else "DISAGREE — classify this before treating either as the record")
        )
    return all_ok, lines
...
def main(argv: list[str]) -> int:
    ...
    parser.add_argument(
        ...
        help="which engine to measure (default: rust, the public default since #262 Stage 3)",
    )
    ...
    for control in controls:
        ok, lines = check(control, args.post_a1, args.engine)
        all_ok &= ok
        for line in lines:
            print(line)
