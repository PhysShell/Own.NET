...
The A2.1 guarded-fact sidecar captures two argument shapes out of the twenty-one
in `probe/case.cs` (the bare identifier and the parenthesized one). The tempting repair — "a handle occurs somewhere below the
...
closure. A completeness checker built on occurrence would confidently prove
...
## Layout
    ...
    probe/expected.json    each method's class (and exclusion), plus what the
                           A2.1 extractor observably emitted for it at dce26ed
