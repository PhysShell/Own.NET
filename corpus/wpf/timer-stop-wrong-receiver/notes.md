...
**Pattern.** Dispose stops only the sibling timer: the WPF002 timer pattern's `Stop()`-based release must
use the same teardown doctrine as `-=` (#278) — a Stop() releases exactly its own receiver; receiver identity keeps the other timer honest.
