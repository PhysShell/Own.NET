...
`pooled`/`native` — movable owned-ресурсы, могут уйти через `return`/`consume`.
...
Элемент буфера зафиксирован как `byte` (как во всех примерах). Бенчмарк-матрица из
дизайн-дока (safe vs unsafe, stack vs pool на размерах 32 B … 1 MB) — это **следующий
слой**: правило «unsafe-backend разрешён только при выигрыше ≥ 10-15 % с
disassembly-обоснованием» здесь задаёт дисциплину, но прогон бенчей вне песочницы.
Unsafe-контракты (`UNS0xx`) пока не реализованы: `native` лоуэрится в
`NativeMemory.Alloc/Free` в `unsafe`-блоке, но pointer-escape проверки — roadmap.
