...
`release`: непересекающиеся времена жизни остаются раздельными (a возвращается до
аренды b), пересекающиеся — вкладываются (release'ы LIFO). Исключение из тела не
«течёт» ни ресурсом, ни буфером. `scratch`/`stack`/`native` динамического размера
guard'ят некорректный (в т.ч. отрицательный) запрос **до** любого trace/counter,
чтобы битый ввод не портил метрики.
