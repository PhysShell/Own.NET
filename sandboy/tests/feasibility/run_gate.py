...
def python_fs_ro_dirs(python_bin):
    ...
    not "can python3 even start". MUST NEVER include "/" itself: a shallow
    interpreter path (e.g. system `/usr/bin/python3`, 2 components below root)
    would otherwise walk all the way up to the filesystem root after a few
    `dirname` hops and silently allowlist the entire filesystem for reads —
    exactly the bug this comment is here to stop from coming back (a real S0
    run once passed every `fs_read_*` denial probe for this reason)."""
    ...
    # stopping well short of the filesystem root.
    ...
    for candidate in ("/opt", "/nix"):
        if os.path.isdir(candidate):
            dirs.add(candidate)
    dirs.discard("/")
    return sorted(p for p in dirs if p != "/" and os.path.isdir(p))
...
class Gate:
    ...
    def record(
        ...
        *,
        ...
        if pass_fail is None:
            ...
            elif category == "credential_canary":
                # env_probe.py reports CLEAN/LEAK, not ALLOWED/DENIED — the
                # "expected" field here just documents that the probe process
                # itself is expected to run to completion either way.
                pass_fail = "pass" if status == "CLEAN" else "fail"
    ...
    def sandboy_refused_to_start(self, run_out):
        return run_out["exit_code"] == 1 and b"Landlock NOT enforced" in run_out["stderr"]
    ...
    def run_existing_smoke(self, demo_sh):
        ...
        p = subprocess.run(
            ["bash", demo_sh],
            cwd=sandboy_repo_root(self.sandboy_bin),
            capture_output=True,
            timeout=60,
            env=env,
        )
    ...
    def fs_tests(self):
        ...
        self.record(
            "fs_unix_socket_bind_outside_allowlist",
            "filesystem",
            "denied",
            "landlock_fs",
        ...
        if wrote_home:
            ...
            self.records.append(
                {
                    "control": "fs_read_host_home_file",
                    "category": "filesystem",
                    "pass_fail": "skipped",
                    "notes": f"could not create {home_target}",
                }
            )

        if wrote_ssh:
            self.record(
                "fs_read_ssh_private_key",
                "filesystem",
                "denied",
                "landlock_fs",
            ...
            self.records.append(
                {
                    "control": "fs_read_ssh_private_key",
                    "category": "filesystem",
                    "pass_fail": "skipped",
                    "notes": f"could not create {ssh_target}",
                }
            )
        ...
        if workspace and os.path.isdir(workspace):
            unrelated = os.path.join(
                os.path.dirname(workspace.rstrip("/")), "sandboy-s0-unrelated-sibling"
            )
            wrote_sibling = False
            try:
            ...
            if wrote_sibling:
                self.record(
                    "fs_read_unrelated_runner_workspace_path",
                    "filesystem",
                    "denied",
                    "landlock_fs",
        ...
        for syscall, layer in (("ptrace", "seccomp"), ("mount", "seccomp"), ("setns", "seccomp")):
            ...
            for depth in ("self", "child", "grandchild", "compiler_tree"):
                ...
                if baseline_status == "DENIED":
                    notes = (
                        "baseline (unsandboxed) ALSO denied this — kernel privilege check "
                    ...
                    notes = (
                        "baseline (unsandboxed) succeeded — a sandboxed denial here "
                        "is attributable to seccomp"
                    )
                self.record(
                    f"syscall_{syscall}_{depth}",
                    "process_inheritance",
                    "denied",
                    layer,
                    out,
    ...
    def env_tests(self):
        ...
        policy = self.write_policy(
            "env", fs_rw=[work]
        )  # note: no env_allow — deny-all is the point
        ...
        for mode in ("stdout", "child"):
            ...
            self.record(
                f"env_canary_absent_{mode}",
                "credential_canary",
                "allowed",
                "sandboy_env_clear",
                run_out,
                notes=(
                    "expected=allowed means the PROBE runs fine; the actual assertion "
                    "is CLEAN vs LEAK in observed_status"
                ),
            )
            # Cross-check: independently grep the dumped file and raw stdout for the
            # literal canary values.
            leaked_in = []
            for name, value in CANARY_ENV.items():
                if value.encode() in run_out["stdout"] or value.encode() in run_out["stderr"]:
    ...
    def net_tests(self):
        ...
        def net(proto, host, port):
            return [
                self.python_bin,
                os.path.join(FIXTURES_DIR, "net_probe.py"),
                proto,
                host,
                str(port),
            ]
        ...
        self.record(
            "net_tcp_connect_denied_no_port_allowlisted",
            "network",
            "denied",
            "landlock_net",
        ...
        self.record(
            "net_tcp_connect_allowed_port_allowlisted",
            "network",
            "allowed",
            "landlock_net",
        ...
        self.record(
            "net_tcp_connect_denied_wrong_port_even_with_allowlist",
            "network",
            "denied",
            "landlock_net",
        ...
        self.record(
            "net_udp_sendto_not_covered_by_landlock",
            "network",
            "gap-not-enforced",
            "not_enforced",
            self.run_sandboxed(policy_deny, net("udp", "127.0.0.1", other_port)),
            notes=(
                "Landlock ABI v4 has no UDP access-control surface at all (TCP "
                "bind/connect only, per README); this is a documented, structural "
                "gap, not a Sandboy regression."
            ),
        )
        self.records.append(
            {
                ...
                "notes": (
                    "Not live-tested against the real 169.254.169.254 address "
                    "(nondeterministic across runner fleets and out of scope per "
                    "'do not contact arbitrary external services'). Landlock ABI v4 "
                    "scopes TCP by PORT only, never by destination address (README: "
                    "'Not host/CIDR/domain egress control'). Therefore ANY policy "
                    "that allowlists a port used by the metadata service (e.g. 80) "
                    "cannot distinguish 169.254.169.254 from any other host on that "
                    "port. Address/CIDR egress scoping is Layer 3 (netns + filtering "
                    "proxy) per the ADR, and is NOT built. This is the same "
                    "structural gap as net_udp_sendto_not_covered_by_landlock, just "
                    "for TCP-by-address instead of UDP-at-all.",
                ),
            }
        )
        ...
        self.records.append(
            {
                ...
                "notes": (
                    "unshare/clone-based namespace creation is not in Sandboy's "
                    "DEFAULT_DENY list (only 'setns' is). On an unprivileged "
                    "non-root runner user this is expected to already fail via the "
                    "kernel's own CAP_SYS_ADMIN / unprivileged_userns_clone check, "
                    "independent of seccomp, so this is not 'plainly broken' — no "
                    "seccomp change made in this gate. Recorded as a finding for "
                    "anyone hardening the denylist later."
                ),
            }
        )
        self.records.append(
            {
                ...
                "pass_fail": "informational_finding",
                "notes": (
                    "setuid(0) on a non-root runner user is denied by the kernel's "
                    "own credential check, not by seccomp (setuid/setgid are not in "
                    "DEFAULT_DENY). Enforcement layer: kernel_permission, not sandboy."
                ),
            }
        )
    ...
    def decide(self, caps, smoke):
        ...
        no_secrets_present = (
            True  # canary values only, never real credentials — enforced by construction
        )
        ...
        criteria = {
            ...
            "symlink_escape_denied": any(
                r["control"] == "fs_symlink_escape_read" and r["pass_fail"] == "pass"
                for r in self.records
            ),
            ...
            # By construction: run() bails on NotEnforced rather than running unconfined.
            "fails_closed_when_mechanism_unavailable": True,
            ...
            rows.append(
                {
                    ...
                    "outer_runner_or_vm_contribution": (
                        "isolation between CI jobs / disposable VM lifecycle only — "
    ...
    def _summary_md(self, caps, smoke, criteria, overall_pass):
        ...
        lines.append(
            "Capability probe only. No third-party repository code was executed. "
        ...
        for k in (
            "kernel_release",
            "arch",
            "landlock_kernel_abi_version",
            "landlock_kernel_abi_note",
            "landlock_ruleset_status",
            "seccomp_installed",
            "no_new_privs_supported",
            "dockerenv_present",
            "cgroup_summary",
        ):
        ...
        lines.append(
            "- Landlock scopes TCP by port only, never by destination address/CIDR — "
        ...
        lines.append(
            "- No resource limits (CPU/memory/disk/process-count/wall-clock) are enforced by "
            "Sandboy itself; any such limits in this gate came from the outer GitHub-hosted VM "
            "and job timeout, not Sandboy."
        )
        lines.append(
            "- `unshare`/`setuid`-family privilege changes are not in the seccomp DEFAULT_DENY "
