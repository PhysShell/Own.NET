...
- **Layer 2 — normalized lowered representation.** A canonical JSON projection
  ...
  This artifact does not exist yet; #259's first deliverable is its Python
  emitter + goldens (the `cfg --format json` precedent: `tests/fixtures/
  cfg_parity.json`, regenerated by `--write`, replayed by Rust).
