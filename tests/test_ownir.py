...
def run() -> int:
    ...
    #     fix it, so the find must survive). Tag [resource: disposable].
    ...
    if len(wfindings) != 1 or wfindings[0].code != "OWN001":
        ...
        if "may not be disposed on every path" not in w0.message \
                or "uow" not in w0.message:
            fails.append(f"uow message wrong: {w0.message!r}")
