...
Слой 1 — СТАТИКА (делаем первым, это основной deliverable)
  ...
  build-free:   own-check, CodeQL (build-mode: none)            [устойчивы к несобираемому solution]
  build-req:    Roslyn analyzer packs, Infer#                    [нужен успешный MSBuild-билд]
  ── весь прогон по target'у идёт на ЛОКАЛЬНОЙ Windows-машине (VS Build Tools + DevExpress 12.2);
     CI самого Own.NET (Linux) гейтит только Python-selftests агрегации, а не аудит target'а ──
...
trap 'git worktree remove --force ../target-audit' EXIT   # tree сносится даже если билд упал
# MSBuild авто-импортит ТОЛЬКО файлы с именами Directory.Build.props/.targets, поэтому
# кладём инъекцию ПОД ЭТИМ именем (не OwnAudit.Directory.Build.*, иначе импорт молча не сработает).
# Если в target'е уже есть свой Directory.Build.props — сохраняем его как *.orig, а наш
# импортит его первой строкой (Condition="Exists(...)"), чтобы не затереть настройки билда.
cp audit/static/inject/OwnAudit.Directory.Build.props   ../target-audit/Directory.Build.props
cp audit/static/inject/OwnAudit.Directory.Build.targets ../target-audit/Directory.Build.targets
msbuild Target.sln /p:OwnAudit=true /p:Configuration=Release \
        /bl:artifacts/build.binlog
# worktree снимается trap'ом на EXIT — временные деревья не накапливаются при падении билда
```
...
| **Build-required** (нужен успешный MSBuild-билд) | NetAnalyzers, Meziantou, Roslynator, AsyncFixer, SonarAnalyzer, IDisposableAnalyzers, WpfAnalyzers, PropertyChangedAnalyzers; Infer# (нужны `.dll`+`.pdb`) | да |

**Решение по окружению (подтверждено):** весь аудит target'а гоняется на **локальной
Windows-машине** (VS Build Tools + DevExpress 12.2) — и build-free, и build-required, и
рантайм. Никакого авто-прогона target'а в CI нет. CI самого Own.NET (Linux) гейтит
**только** Python-selftests слоя агрегации (как `oracle_compare --selftest` сейчас), а не
аудит легаси. Tiers здесь — это **про устойчивость**: build-free даёт каркас отчёта даже
когда solution не собирается; build-required добавляет глубину, когда билд прошёл.
Артефакты/отчёты коммитятся в репозиторий. NDepend выпадает (нет лицензии) —
архитектурное ранжирование идёт на free-метриках (Roslynator/Sonar/CodeQL), помечается
в coverage.
...
# OWN001 — ЗОНТИЧНЫЙ leak-код: один ruleId на IDisposable-поля/локалы, subscription и timer.
# Различает их тег [resource: …] в тексте диагностики. Плоский маппинг OWN001->кат.1
# схлопнул бы subscription/timer-утечки (кат. 2/3) в IDisposable и недосчитал бы их —
# поэтому normalize.py парсит тег и разводит по подтипу:
"OWN001":
  by_resource:                                       # ключ = [resource: …]
    "subscription token": {category: 2, name: "subscription-leak"}
    "timer":              {category: 3, name: "timer-leak"}
    "*":                  {category: 1, name: "idisposable-leak"}   # disposable field/local
"OWN014":           {category: 2, name: "region-escape"}   # escape в более долгоживущий
                                                           # регион (vm->App, SystemEvents);
                                                           # НЕ generic subscription-leak
...
## 6. Обратная связь: аудит кормит OwnLang (поздний трек, ВНЕ первого deliverable)

> **Scope-пометка (подтверждено):** готовый результат первого шага — это **только
> health-отчёт по легаси** (§9, Фаза 1). Описанное ниже — *поздний отдельный трек*
> (Фаза 4), а не часть первой поставки. Записано, чтобы возможность не испарилась.

«Натаскать наш анализатор» получается потом почти бесплатно как побочка:
...
audit/                              # cohesive; ничего не импортирует из ownlang/
  ...
  static/
    ...
    tools/                          # по одному тонкому runner'у; каждый → SARIF
      roslyn_pack.ps1  codeql.sh  infersharp.sh  owncheck.py
      # (без ndepend.ps1 — NDepend исключён принятым решением «только free-стек», §8/§11)
    inject/
...
|---|---|---|
| **Весь аудит target'а — на локальной Windows-машине** (нет авто-прогона в CI) | результат зависит от того, что машину держат в форме (DevExpress, SDK) | принято осознанно: запуск локальный и воспроизводимый по коммиту; CI Own.NET гейтит только Python-selftests агрегации; build-free tier даёт каркас даже на несобираемом solution |
...
- 1b: runner'ы build-free (own-check, CodeQL) → SARIF (каркас отчёта даже на несобираемом solution).
- 1c: runner'ы build-required (Roslyn-пак, Infer#) на собравшемся билде; `continue-on-error`;
  SARIF кладётся в `artifacts/own-audit/`. Всё локально на Windows-машине.
...
- **Deliverable (= done-критерий шага 1): health-отчёт по легаси** — категоризированный,
  с ранжированием «где хуже / где почти норм» + честной coverage-картой. **Только это.**
  Фид находок обратно в OwnLang (§6) в первый deliverable НЕ входит — отдельный поздний трек.
...
**Фаза 4 — Вынос и обратная связь (S, поздний трек, ВНЕ первого deliverable)**
...
1. **Окружение — только локальная Windows-машина.** Весь аудит target'а (build-free,
   build-required, рантайм) гоняется локально на Windows (VS Build Tools + DevExpress
   12.2). Авто-прогона target'а в CI нет; CI Own.NET (Linux) гейтит только Python-selftests
   агрегации. Tiers — про устойчивость к несобираемому solution, не про машину. (§1, §3.2, §8, §9)
2. **Готовый результат первого шага — только health-отчёт по легаси.** Категоризированный,
   с ранжированием «где хуже/почти норм» и честной coverage-картой. Фид находок обратно в
   OwnLang (§6) — поздний отдельный трек (Фаза 4), ВНЕ первой поставки. (§6, §9)
3. **Язык оркестрации статики/агрегации — Python.** Переиспользуем `parse_sarif` и
   oracle-логику, нулевой coupling к ядру, OS-agnostic. C# — только в рантайме (FlaUI,
   ClrMD). (§7)
4. **Только free-стек, без коммерции.** Нет NDepend → архитектурное ранжирование
...
5. **DevExpress — baseline-suppress целиком.** Находки в `DevExpress.*` глушатся на
...
- Точный список топ-N экранов для рантайм-сценариев Фазы 2 (тяжёлый справочник,
  ...
  Фазой 2 (рантайм — это уже не первый deliverable).
